...
startBtn.addEventListener('click', ()=>{
    ...
    if (game.gameStatus === 'idle') game.start();
    else game.restart();
...
document.addEventListener('keydown', (e)=>{
    if (game.gameStatus === 'idle') return;
    const score = document.querySelector('.game-score');
    switch(e.key){
        case 'ArrowUp':
            game.moveUp();
 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    /* eslint-disable no-shadow */ addNewTile() {
        const emptyCells = [];
        for(let i = 0; i < this.gameBoard.length; i++){
            for(let j = 0; j < this.gameBoard.length; j++)if (this.gameBoard[i][j] === 0) emptyCells.push({
                i,
                j
            });
        }
    ...
    /* eslint-disable no-shadow */ renderGameBoard(board) {
    ...
    slideLeft(row) {
        ...
        while(newRow.length < this.gameBoard.length)newRow.push(0);
        return newRow;
    }
    moveLeft() {
    ...
    moveRight() {
        const oldGameBoard = JSON.stringify(this.gameBoard);
        const newGameBoard = this.gameBoard.map((row)=>this.slideLeft([
                ...row
            ].reverse()).reverse());
        if (JSON.stringify(newGameBoard) !== oldGameBoard) {
            this.gameBoard = newGameBoard;
    ...
    moveDown() {
        const oldGameBoard = JSON.stringify(this.gameBoard);
        let transposed = this.matrixTranspose(this.gameBoard);
        transposed = transposed.map((row)=>this.slideLeft([
                ...row
            ].reverse()).reverse());
        const newGameBoard = this.matrixTranspose(transposed);
        if (JSON.stringify(newGameBoard) !== oldGameBoard) {
   ...
   */ getStatus() {
        let canMerge = false;
        let hasEmpty = false;
        for(let i = 0; i < this.gameBoard.length; i++)for(let j = 0; j < this.gameBoard.length; j++){
            const currentCell = this.gameBoard[i][j];
            if (currentCell === 2048) {
                this.gameStatus = 'win';
                return;
            }
            if (currentCell === 0) hasEmpty = true;
            if (j < this.gameBoard.length - 1 && currentCell === this.gameBoard[i][j + 1]) canMerge = true;
            if (i < this.gameBoard.length - 1 && currentCell === this.gameBoard[i + 1][j]) canMerge = true;
        }
        if (hasEmpty || canMerge) this.gameStatus = 'playing';
